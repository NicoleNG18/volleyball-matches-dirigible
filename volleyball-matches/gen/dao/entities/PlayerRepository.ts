...
export class PlayerRepository {
    ...
    private static readonly DEFINITION = {
        table: "VOLEYBALL_MATCHES_PLAYER",
        properties: [
            {
                name: "Id",
                column: "PLAYER_ID",
    ...
    public customDataCount(): number {
        const resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "VOLEYBALL_MATCHES_PLAYER"');
    ...
    private async triggerEvent(data: PlayerEntityEvent | PlayerUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("volleyball-matches-entities-Player", ["trigger"]);
        ...
        producer.topic("volleyball-matches-entities-Player").send(JSON.stringify(data));
    }
}
